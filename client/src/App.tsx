...
import { Building, Edge, parseBuildings, parseEdges } from './buildings';
...
export class App extends Component<AppProps, AppState> {
  ...
  doEndPointChange = (endPoints?: [Building, Building]): void => {
    ...
    if (endPoints) {
      ...
      console.log(`show path from ${start.longName} to ${end.longName}`);
      ...
      fetch(url)
        ...
        .catch((e) => console.error(`api/shortestPath error response does not work: ${e}`));
    } else {
  ...
  doPathResp = (res: Response): void => {
    if (res.status === 200) {
      res.json()
        ...
        .catch(() => this.doPathError("200 response is not valid JSON"));
    } else if (res.status === 400) {
      res.json()
        .then(this.doPathError)
        .catch(() => this.doPathError("400 response is not text"));
    } else {
      this.doPathError(`bad status code ${res.status}`);
    }
  };

  doPathJson = (val: unknown): void => {
    console.log("Received response in doPathJson:", val);
  
    if (!isRecord(val)) {
      console.error("Response is not a record:", val);
      return;
    }
  
    const pathObj = val.steps;
    if (!isRecord(pathObj)) {
      console.error("val.steps is not a valid Path object:", pathObj);
      return;
    }
  
    const steps = pathObj.steps;
    if (!Array.isArray(steps)) {
      console.error("pathObj.steps is not an array:", steps);
      return;
    }
  
    const pathEdges = parseEdges(steps);
    this.setState({ path: pathEdges });
  };
